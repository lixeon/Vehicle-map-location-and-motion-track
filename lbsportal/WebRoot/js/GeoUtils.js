    ...
    * 主入口类是<a href="symbols/BMapLib.GeoUtils.html"/*tpa=http://www.beidouguard.com:6010/js/symbols/BMapLib.GeoUtils.html*/>GeoUtils</a>， 
    * 基于Baidu Map API 1.2。 
    * 
...
var degreesPerRadian = 180.0 / Math.PI;  
      ...
      GeoUtils.GetDistance=function(lat1,lon1,lat2,lon2)
{
//   var radLat1 = lat1;
//   var radLat2 = lat2;
...
//var s = 6378000.137*Math.acos(1-(Math.sin(a)*Math.sin(a)+Math.sin(b)*Math.sin(b)+(Math.cos(a)-Math.cos(b))*(Math.cos(a)-Math.cos(b))))；
 var radLat1 = lat1;  
        var radLat2 = lat2;  
        ...
        GeoUtils.getPolygonArea = function (polygon) {  
            ...
            if (surpluspoint.length!=0) {
             ...
             if (surpluspoint.length==1) {
                if (Count%2!=0) {
                    LowX = pts[Count-1].lng ;   
                    LowY = pts[Count-1].lat ;  
                    MiddleX = pts[Count-2].lng ;  
                    MiddleY = pts[Count-2].lat;  
                    HighX = pts[icount].lng ;  
                    HighY = pts[icount].lat ;  
                }
                else {
                    LowX = pts[Count-1].lng;   
                    LowY = pts[Count-1].lat ;  
                    MiddleX = pts[(Count-1+icount)/2].lng ;  
                    MiddleY = pts[(Count-1+icount)/2].lat;  
                    HighX = pts[icount].lng ;  
                    HighY = pts[icount].lat ;  
                } 
                var aarr = [];
                var barr = [];
                var carr = [];
                aarr[0] = new BMap.Point(LowX, LowY);
                aarr[1] = new BMap.Point(MiddleX, MiddleY);
                barr[0] = new BMap.Point(MiddleX, MiddleY);
                barr[1] = new BMap.Point(HighX, HighY);
                carr[0] = new BMap.Point(HighX, HighY);
                carr[1] = new BMap.Point(LowX, LowY);
                var a = BMapLib.GeoUtils.getPolylineDistance(aarr);
                var b = BMapLib.GeoUtils.getPolylineDistance(barr);
                var c = BMapLib.GeoUtils.getPolylineDistance(carr);
//var a = BMapLib.GeoUtils.GetDistance(LowY,LowX, MiddleY,MiddleX );
//var b = BMapLib.GeoUtils.GetDistance(MiddleY,MiddleX, HighY,HighX);
//var c = BMapLib.GeoUtils.GetDistance(HighY,HighX, LowY,LowX);
                var p =(a+b+c)/2;
                var d = p*(p-a)*(p-b)*(p-c);
                if (d<0) {
                    d = 0-d;
            ...
            for (var i = icount; i < Count; i+=addicount) {  
                ...
                if (i == icount) {  
                    ...
                    MiddleX = pts[i].lng ;  
                    MiddleY = pts[i].lat ;  
                    HighX = pts[i+addicount/2].lng ;  
                    HighY = pts[i+addicount/2].lat ;  
                }  
                else if (i == Count +icount-addicount) {  
                    LowX = pts[i-addicount+1].lng ;  
                    LowY = pts[i - addicount+1].lat ;  
                    MiddleX = pts[i].lng ;  
                    MiddleY = pts[i].lat ;  
                    HighX = pts[Count-1].lng ;  
                    HighY = pts[Count-1].lat ;  
                }  
...
//                if (Math.sqrt(AHtangent * AHtangent + BHtangent * BHtangent + CHtangent * CHtangent) * Math.sqrt(ALtangent * ALtangent + BLtangent * BLtangent + CLtangent * CLtangent)==0) {
//    AngleCos=1;
//}
//else
//{
...
//            }  
//            else {  
//                 Sum = Sum2+(2*Math.PI*Count1-Sum1);
//            }  
//            totalArea = (Sum - (Count - 2) * Math.PI) * Radius * Radius;  
//            if (totalArea<0) {
//    totalArea = 0-totalArea;
//}
            //totalArea = totalArea/1000000000
