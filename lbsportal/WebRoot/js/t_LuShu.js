 ...
 * 用户可以在地图上自定义轨迹运动
 * 可以自定义路过某个点的图片，文字介绍等。
 * 主入口类是<a href="symbols/BMapLib.LuShu.html"/*tpa=http://www.beidouguard.com:6010/js/symbols/BMapLib.LuShu.html*/>LuShu</a>，
 * 基于Baidu Map API 1.2。.
 *
...
(function() {
     ...
     */
     BMapLib.T_LuShu = function(map, path, opts) {
        ...
        //如果不是默认实例，则使用默认的icon
        if (!this._opts.icon instanceof TIcon) {
    ...
    LuShu.prototype.start = function() {
        ...
        if (me.i && me.i < len - 1) {
            ...
            if (!me._fromPause) {
	            ...
	            me._moveNext(me.i, me._currentCount);
            }
        }else {
            //第一次点击开始，或者点了stop之后点开始
            me._addMarker(me._pos);
    ...
    baidu.object.extend(LuShu.prototype, {
        ...
        _addMarker: function(pos) {
            ...
            var marker = new TMarker(pos || this._path[0]);
        ...
        _getDistance: function(pxA, pxB) {
            return Math.sqrt(Math.pow(pxA.getMercatorLng() - pxB.getMercatorLng(), 2) + Math.pow(pxA.getMercatorLat() - pxB.getMercatorLat(), 2));
        },
         ...
         * @return 无返回值.
         */
        _move: function(initPos,targetPos,effect, curCount) {
            //自动移动地图
            var pts = [initPos, targetPos],
                view = this._map.getViewport(pts);
            //this._map.centerAndZoom(view.center, view.zoom);
            this._map.setCenter(view.center);

            var me = this,
                //当前的帧数
                currentCount = curCount || 0,
                //步长，米/秒
                timer = 10,
                timer2 = 100,
            ...
            //两点之间当前帧数大于总帧数的时候，则说明已经完成移动
	            if (currentCount >= count) {
	                ...
	                me._moveNext(++me.i);
	                    ...
	                    //正在移动
	                    currentCount++;
	                    var x = effect(initPos.getMercatorLng(), targetPos.getMercatorLng(), currentCount, count),
	                        y = effect(initPos.getMercatorLat(), targetPos.getMercatorLat(), currentCount, count),
	                        arr = me._map.mercatorToWGS84(x, y),
	                        pos = new TLngLat(arr[0], arr[1]);
	                        	                       
	                    //设置marker
	                    me._marker.setLngLat(pos);
	                    //设置自定义overlay的位置
	                    me._setInfoWin(pos);
         ...
         * @return 无返回值.
         */
        _moveNext: function(index,curCount) {
            var me = this;
            if (index < this._path.length - 1) {
                me._move(me._path[index], me._path[index + 1], me._tween.linear, curCount);
            } else {
                 me._overlay.setHtml(me._opts.defaultContent);//最后设置为默认内容
                 me._overlay.KY();
            }
        },
        /**
        ...
        _troughPointIndex: function(markerPoi) {
            ...
            for (var i = 0, len = t.length; i < len; i++) {
                ...
                if (!t[i].bShow) {
                    distance = this._getDistance(new TLngLat(t[i].lng, t[i].lat), markerPoi);
                    //两点距离小于10米，认为是同一个点
                    if (distance < 1) {
                        t[i].bShow = true;
                        return i;
    ...
    CustomOverlay.prototype = new TOverlay();
    ...
    CustomOverlay.prototype.getObject = function(){
        return this._div;
    }

    CustomOverlay.prototype.KY = function() {//nt加密前为reDraw
        this.setPosition(this.lushuMain._marker.getLngLat(), this.lushuMain._marker.getIcon().getSize());
    }
    ...
    baidu.object.extend(CustomOverlay.prototype, {
        ...
        setPosition: function(poi,markerSize) {
            ...
            var px = this._map.fromLngLatToDivPixel(poi),
				...
				overlayW = parseInt(this._div.offsetWidth),
                overlayH = parseInt(this._div.offsetHeight);
			
            this._div.style.left = (px.x - overlayW / 2)+ 'px';
            this._div.style.top = (px.y - markerSize.height - overlayH) + 'px';
        },
        //设置overlay的内容
